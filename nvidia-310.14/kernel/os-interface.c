...
{
    NvU64 cpu_hz = 0;
#if defined(CONFIG_CPU_FREQ) && !(defined(KERNEL_2_4) || defined(NV_VMWARE))
...
/*
 * T20/T30 map PCIe enhanced config space separately from basic config
 * space, so the functions below are used to access enhanced space through
 * direct kernel calls.
 *
 * For architectures where we have other means of accessing enhanced config
 * space and the kernel pci_read/write_config_* functions may
 * not allow access beyond the basic PCI config space, be conservative and
 * limit access to basic config space.
 * 
 */
#if defined(NVCPU_ARM)
#define NV_PCIE_CFG_MAX_OFFSET 0x1000
#else
#define NV_PCIE_CFG_MAX_OFFSET 0x100
#endif
...
#if defined(NVCPU_ARM)
    CACHE_FLUSH_ALL();
    /* flush the external L2 cache in cortex-A9 */  
    OUTER_CACHE_FLUSH();
    return RM_OK;
#endif
    return RM_ERR_NOT_SUPPORTED;
                                          ...
                                          NvU32 flags)
{
    ...
    if (flags & NV_FLUSH_USER_CACHE)
    {
        // Flush L1 cache
        cpu_cache.dma_flush_range((void *)(NvU32)start, (void *)(NvU32)end);
 
        // Now flush L2 cache.
        outer_cache.flush_range((NvU32)physStart, (NvU32)physEnd);
    }

    if (flags & NV_INVALIDATE_USER_CACHE)
    {
        // Invalidate L1 cache
        cpu_cache.dma_map_area((void *)(NvU32)start, (NvU32)(end - start), DMA_FROM_DEVICE);

        // Invalidate L2 cache
        outer_cache.inv_range((NvU32)physStart, (NvU32)physEnd);
    }
    return RM_OK;
#else
    return RM_ERR_NOT_SUPPORTED;
...
{
#if defined(NV_EFI_ENABLED_PRESENT)
    return efi_enabled;
#else
    return FALSE;
#endif
