 ...
 *
 * This function relies on the ability to perform kernel virtual address to PFN
...
static int __init nvidia_init_module(void)
{
    ...
    if (rc < 0)
    {
        nv_printf(NV_DBG_ERRORS, "NVRM: failed to create mempool!\n");
        return rc;
    }
...
)
{
    ...
    if (nvfp->event_head != NULL)
    {
        ...
        if ((nvet->event.flags & NV_EVENT_FLAGS_DATA_VALID) == 0)
        {
...
)
{
    ...
    if (data_valid)
        nvet->event.flags |= NV_EVENT_FLAGS_DATA_VALID;
    ...
    else
    {
        ...
        if (rm_is_supported_device(sp, nv) != RM_OK)
        {
            nv_printf(NV_DBG_ERRORS,
                ...
                NV_VERSION_STRING, NV_KERNEL_NAME, NV_KERNEL_NAME);
            goto err_not_supported;
        }
...
{
#if defined(NV_CHANGE_PAGE_ATTR_PRESENT) || \
  defined(NV_SET_PAGES_UC_PRESENT) || defined(NV_SET_MEMORY_UC_PRESENT)
...
)
{
    ...
    switch (pci_state)
    {
        ...
        case PCI_D0:
            ...
            tasklet_init(&lnv->tasklet, nv_kern_isr_bh, (NvUPtr)NV_STATE_PTR(lnv));
            status = rm_power_management(sp, nv, 0, power_state);
            break;
